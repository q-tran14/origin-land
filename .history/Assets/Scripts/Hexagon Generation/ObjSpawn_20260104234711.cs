...
// ===== BƯỚC 3: SPAWN OBJECT DỰA TRÊN TILE =====
...
if (Physics.Raycast(rayStart, Vector3.down, out RaycastHit hit, 50f))
{
    ...
    spawnedObjects.Add(objData);
                ...
                // ----- Instantiate trong Scene (nếu muốn) -----
                if (instantiateInScene)
                {
                    Transform parent = objectHolder != null ? objectHolder : holderObject != null ? holderObject.transform : null;
                    GameObject instance = Instantiate(prefabToSpawn, spawnPos, Quaternion.identity, parent);
                    instance.name = string.IsNullOrEmpty(prefabName) ? prefabToSpawn.name : prefabName;
                }
