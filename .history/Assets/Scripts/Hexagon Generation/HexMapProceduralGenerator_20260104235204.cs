...
public class HexMapProceduralGenerator : MonoBehaviour
{
    ...
    private void LoadMapFromSavedData(MapSaveData data)
    {
        ...
        // ===== BƯỚC 4: LOAD OBJECT (CÂY, ĐÁ, WEAPONRACK...) =====
        if (data.objects != null && data.objects.Length > 0)
        {
            GameObject objectHolder = new GameObject("ObjectHolder");
            objectHolder.transform.parent = holderObject.transform;

            foreach (ObjectData obj in data.objects)
            {
                // Lấy tile tương ứng
                if (obj.tileX < 0 || obj.tileX >= mapWidth ||
                    obj.tileZ < 0 || obj.tileZ >= mapHeight)
                    continue;

                TileData tile = mapData[obj.tileX, obj.tileZ];
                if (tile == null || tile.tileGO == null) continue;

                // Tìm prefab object
                GameObject prefab = objSpawner.obj
                    .FirstOrDefault(o => o.asset != null && o.asset.name == obj.prefabName)
                    ?.asset;

                if (prefab == null)
                {
                    Debug.LogWarning($"❌ Không tìm thấy object prefab: {obj.prefabName}");
                    continue;
                }

                // Tính vị trí spawn dựa trên tile
                ...
                Instantiate(
                    prefab,
                    pos,
                    Quaternion.Euler(0, obj.yRotation, 0),
