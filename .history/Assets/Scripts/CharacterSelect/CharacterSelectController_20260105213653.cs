...
public class CharacterSelectController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        DisableGameplay(barbarianModel);
        DisableGameplay(mageModel);
    }
    private void DisableGameplay(GameObject model)
    {
        var movement = model.GetComponent<CharacterMovement>();
        if (movement != null)
        {
            movement.enabled = false;
            Debug.Log($"{model.name} CharacterMovement DISABLED");
        }
    }
    // ================= CHARACTER SELECT =================
    ...
    {
        Debug.Log("SELECT BARBARIAN CALLED");
        selectedCharacter = CharacterType.Barbarian;
        barbarianAnimator.Play("Cheer", 0, 0f);
